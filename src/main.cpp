...
{  
	...
	bool encrypting;
	if ( strcmp(argv[1], "-e")==0 ) { encrypting = true; }
	else if ( strcmp(argv[1], "-d")==0 ) { encrypting = false; }
	else {
	...
	const unsigned int N = 256; // Range of accepted values
	std::array<unsigned char, N> password; // Password
	...
	Coder<N>* coder;
	if (encrypting) {
		coder = new Encoder<N>(password);
	}
	else {
		coder = new Decoder<N>(password);
	}
	...
	for (int i = 0; i < T; ++i) {
		...
		for (unsigned int l = 0; l < N; ++l) {
			password[l] = (password[l] + caesar) % N;
		}
		
		// Set the encoder key
		coder->changeKey(password);
	}
